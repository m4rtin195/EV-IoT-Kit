...
void Logger::logRequest(bool brief)
{
    ...
    if(brief)
    {
        ...
        cout << s.arg(time().c_str())
                .arg(QString::number(v->voltage, 'f', 1))
                .arg(QString::number(v->charge, 'f', 1))
                .arg(QString::number(v->target_charge, 'f', 0))
                .arg(minsToTime(v->elapsed_time).c_str())
                .arg(minsToTime(v->remaining_time).c_str())
                .arg(v->range)
                .toStdString();
        cout << endl;
    }
    ...
    else
    {
        ...
        printf("  Capacity: %dWh \t Max capacity: %dWh \t Factory capacity: %dWh \t Wear: %.1f% \n", 50000, 0/*actual_capacity*/, v->battery_capacity, 0/*(actual_capacity/factory_capacity)*/);
